    ...
    return (
        <div className="max-w-4xl mx-auto p-6">
            <div className="flex justify-between items-center mb-8">
                ...
                <Link href="/books/new" className="hover:opacity-60 transition-opacity">+ Add a new book</Link>
            </div>
                ...
                {books.map((book) => (
                    <li key={book.id}>
                        <Link href={`/books/${book.id}`} className="group">
                            {book.img_url ? (
                                <img src={book.img_url} alt={book.title} className="w-full aspect-[2/3] object-cover rounded group-hover:opacity-80 transition-opacity" />
                            ) : (
