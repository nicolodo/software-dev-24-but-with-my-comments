...
export default async function SingleBookPage({ params }) {
    ...
    const book = (await db.query(`SELECT * FROM books WHERE id = $1`, [id])).rows[0]
    ...
    const reviews = (await db.query(`
        ...
        WHERE review.book_id= $1;`, [id])).rows
    ...
    async function handleSubmitReview(formData) {
        ...
        await db.query(`insert into review (user_id, book_id, content) values ($1, $2, $3)`, [user[0].id, id, content])
    ...
    return (
        <div className="max-w-3xl mx-auto p-6">
            <div className="flex gap-8 mb-10">
                {book.img_url && (
                    <img src={book.img_url} alt={book.title} className="w-48 aspect-[2/3] object-cover rounded shrink-0" />
                )}
                <div>
                    <h1 className="text-3xl">{book.title}</h1>
                    <p className="opacity-60 mt-1">by {book.author}</p>
                    {book.released && <p className="text-sm opacity-40 mt-1">{new Date(book.released).toLocaleDateString()}</p>}
                    {book.description && <p className="mt-4">{book.description}</p>}
                    {book.quote && <p className="mt-4 italic opacity-70">"{book.quote}"</p>}
                </div>
            </div>

            <h2 className="text-xl mb-3">Leave a Review</h2>
            <form className="mb-10" action={handleSubmitReview}>
                <textarea name="content" placeholder="Write your review..." required className="w-full border rounded p-3 resize-none h-24" />
                <button type="submit" className="mt-2 px-4 py-2 bg-black text-white rounded hover:opacity-80 transition-opacity cursor-pointer">Submit Review</button>
            </form>
